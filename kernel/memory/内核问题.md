...
### 在 Dirty COW内存漏洞中，如果 Dirty COW程序没有madviseThread线程，即只有procselfmemThread 线程，能否修改foo文件的内容呢?

不，如果Dirty COW程序只有procselfmemThread线程而没有madviseThread线程，它将无法修改foo文件的内容。在Dirty COW漏洞中，madviseThread线程用于使内存页面从"写时复制"模式切换到"可写"模式，从而使procselfmemThread线程可以修改共享内存。如果只有procselfmemThread线程，它无法切换内存页面的模式，因此无法修改文件内容。



### 假设在内核空间获取了某个文件对应的 page cache页面的struct page 数据结构，而对应的VMA属性是只读，那么内核空间是否可以成功修改该文件呢?
