...
第21~34行代码，设置cc->migrate_pn和cc->freepfn。简单来说，cc->migrate pfn设置为zone的开始pfn（zone->zone_start pfn)，表示从zone的第一个页面开始扫描和查找哪些页面可以被迁移。cc->freepfn设置为zone的最末的pfn，表示从zone的最末端开始扫描和查找有哪些空闲的页面可以用作迁移页面目的地。
